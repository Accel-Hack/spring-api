...
public class LoggerInterceptor implements HandlerInterceptor {
  ...
  public boolean preHandle(@NonNull HttpServletRequest request,
    ...
    log.info(
        String.format("[%-5s(session-%s)] %-4s %s ip:%s", "BEGIN", request.getSession().getId(),
            request.getMethod(), request.getRequestURI(), getRemoteAddr(request)));
    begin = Instant.now();
  ...
  public void afterCompletion(@NonNull HttpServletRequest request,
    ...
    log.info(String.format("[%-5s(session-%s)] took %sms.", "END", request.getSession().getId(),
        durationMs));
  }
  ...
  private void addLog(@NonNull HttpServletRequest request, int status, double executeTime,
      Exception ex) {
    try {
      ApiLogDto apiLog = ApiLogDto.builder().operationTime(begin)
          .operator(authRegistry.getOperator()).method(request.getMethod())
          .path(request.getRequestURI()).query(request.getQueryString())
          .requestBody(request.getReader().lines().collect(Collectors.joining()))
          .remoteIp(getRemoteAddr(request)).executionTimeMs(executeTime).status(status)
          .exception(Objects.nonNull(ex) ? ex.getLocalizedMessage() : "").build();
